...
>>>>>>> fc910eb3082c08e9c11a043ecc7fa3aee3e16544
...
RGE <- function(disease_index) {
  y_full <- prediction_matrices[[disease_index]]$y_hat
  y_all_reduced <- map(reduced_matrix[[disease_index]],
    \(y_reduced) {
      1 - RG(
        y_full,
        y_reduced
  ...
  as_tibble(y_all_reduced)
}

model_explainabilities <- map(1:length(prediction_matrices), RGE)
names(model_explainabilities) <- names(prediction_matrices)
